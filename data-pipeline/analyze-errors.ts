...
import { harmonyOf, vowelsOf } from "../packages/i18n-inflect/src/hu/phonology.js";
...
function compoundHead(lemma: string): string | undefined {
  ...
  for (let i = 1; i < lemma.length - 1; i++) {
    ...
    if (tail.length >= 2 && STEM_FLAGS.has(tail) && (best === undefined || tail.length > best.length)) {
      best = tail;
    }
  }
  return best;
...
for (const [lemma, forms] of groupByLemma(heldOut)) {
  for (const [tag, accepted] of forms) {
    ...
    const got = inflectNounRules(lemma, STEM_FLAGS.get(lemma), plural, huCase);
    ...
    else if (head) cls = `összetett szó — ismert utótag (${head})`;
...
for (const [cls, e] of [...classes].sort((a, b) => b[1].forms - a[1].forms)) {
  console.log(`${((100 * e.forms) / total).toFixed(2)}%  ${e.forms.toString().padStart(5)} alak  ${e.lemmas.size.toString().padStart(4)} lemma  ${cls}`);
